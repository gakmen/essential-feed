...
//  EssentialAppTests
//
//  Created by Георгий Акмен on 07.09.2023.
//
...
class FeedAcceptanceTests: XCTestCase {
    ...
    func test_onLaunch_displaysRemoteFeedWhenCustomerHasConnectivity() throws {
        ...
        XCTAssertEqual(feed.numberOfRenderedFeedViews(), 3)
        XCTAssertEqual(feed.renderedFeedImageData(at: 0), makeImageData0())
        XCTAssertEqual(feed.renderedFeedImageData(at: 1), makeImageData1())
    ...
    func test_onLaunch_displaysRemoteCachedFeedWhenCustomerHasNoConnectivity() throws {
        let sharedStore = try CoreDataFeedStore.empty
        let onlineFeed = launch(httpClient: .online(response), store: sharedStore)
        onlineFeed.simulateImageViewVisible(at: 0)
        onlineFeed.simulateImageViewVisible(at: 1)
        onlineFeed.simulateLoadMoreFeedAction()
        onlineFeed.simulateImageViewVisible(at: 2)
        
        let offlineFeed = launch(httpClient: .offline, store: sharedStore)

        XCTAssertEqual(offlineFeed.numberOfRenderedFeedViews(), 3)
  ...
  private func enterBackground(with store: CoreDataFeedStore) {
    ...
    private func makeData(for url: URL) -> Data {
        ...
        case "/essential-feed/v1/feed" where url.query?.contains("after_id=A28F5FE3-27A7-44E9-8DF5-53742D0E4A5B") == true :
    ...
    private func makeSecondPageFeedData() -> Data {
        return try! JSONSerialization.data(withJSONObject: ["items": [
            ["id": "A28F5FE3-27A7-44E9-8DF5-53742D0E4A5B", "image": "http://feed.com/image-2"]
        ]])
    }
